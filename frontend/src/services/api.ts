...
import { AuthResponse, Curriculum, Dashboard, Lesson, Quiz, QuizResult, Resource, Project, SocialConnection } from '../types';
...
export const socialAPI = {
  getConnections: (): Promise<{ connections: SocialConnection[] }> =>
    api.get('/social/connections').then(res => res.data),
  
  connectAccount: (platform: string, username: string, walletAddress?: string): Promise<{ message: string; connections: SocialConnection[] }> =>
    api.post('/social/connect', { platform, username, walletAddress }).then(res => res.data),
  
  disconnectAccount: (platform: string): Promise<{ message: string; connections: SocialConnection[] }> =>
    api.delete(`/social/disconnect/${platform}`).then(res => res.data),
  
  verifyCampNetwork: (campNetworkId: string): Promise<{ message: string; campNetworkId: string }> =>
    api.post('/social/camp-network/verify', { campNetworkId }).then(res => res.data),
};
